...
class train_pair_transform(object):
    def __init__(self, cfg_data, check_dim=True):
        ...
        self.scale_to_setting = own_transforms.ScaleByRateWithMin(
            cfg_data.TRAIN_SIZE[1], cfg_data.TRAIN_SIZE[0]
        )
    ...
    def __call__(self, img, target):
        ...
        self.c_h, self.c_w = int(self.cfg_data.TRAIN_SIZE[0] / self.scale_factor), int(
            self.cfg_data.TRAIN_SIZE[1] / self.scale_factor
        )
        img, target = check_image(
            img,
            target,
            (self.c_h, self.c_w),
        ...
        if self.pair_flag % 2 == 1:
            if self.check_dim:
                x1 = max(
                    0, int(self.last_crop_left[0] + (self.last_cw_ch[0] - self.c_w))
                )
                y1 = max(
                    0, int(self.last_crop_left[1] + (self.last_cw_ch[1] - self.c_h))
                )
        ...
        img, target = self.resize_and_crop(
            img, target, self.crop_left, crop_size=(self.c_h, self.c_w)
        )
        img, target = self.scale_to_setting(img, target)

        target["points"][:, 0] = torch.clamp(
            target["points"][:, 0], min=0, max=img.size[0] - 1
        )
        target["points"][:, 1] = torch.clamp(
            target["points"][:, 1], min=0, max=img.size[1] - 1
        )
...
class test_transform(object):
    ...
    def __call__(self, img, target):
        ...
        short_side = min(w, h)
        if (
            self.cfg_data.TEST_MAX_LONG is not None
            and self.cfg_data.TEST_MAX_SHORT is not None
        ):
            max_long_side, max_short_side = (
                self.cfg_data.TEST_MAX_LONG,
                self.cfg_data.TEST_MAX_SHORT,
            )
...
class train_resize_transform(object):
    ...
    def __call__(self, img, target):
        ...
        target["points"][:, 0] = torch.clamp(
            target["points"][:, 0], min=0, max=self.new_w - 1
        )
        target["points"][:, 1] = torch.clamp(
            target["points"][:, 1], min=0, max=self.new_h - 1
        )
...
class train_transform(object):
    def __init__(self, cfg_data, check_dim=True):
        ...
        self.scale_to_setting = own_transforms.Scale(
            cfg_data.TRAIN_SIZE[1], cfg_data.TRAIN_SIZE[0]
        )
        self.check_dim = check_dim
    ...
    def __call__(self, img, target):
        img, target = check_image(
            img, target, (self.final_h, self.final_w)
        )  # make sure the img size is large than we needed
        ...
        y1, y2 = max(0, y_crop_center - self.final_h // 2), min(
            h_ori, y_crop_center + self.final_h // 2
        )
        x1, x2 = max(0, x_crop_center - self.final_w // 2), min(
            w_ori, x_crop_center + self.final_w // 2
        )
        self.crop_left = (x1, y1)

        img, target = self.crop(
            img, target, self.crop_left, crop_size=(y2 - y1, x2 - x1)
        )
        w, h = img.size
        ...
        target["points"][:, 0] = torch.clamp(
            target["points"][:, 0], min=0, max=self.final_w - 1
        )
        target["points"][:, 1] = torch.clamp(
            target["points"][:, 1], min=0, max=self.final_h - 1
        )
...
def createTrainData(datasetname, Dataset, cfg_data, distributed):
    ...
    resize_transform = train_resize_transform(
        cfg_data.TRAIN_SIZE[0], cfg_data.TRAIN_SIZE[1]
    )
    train_set = Dataset(
        cfg_data.TRAIN_LST,
        cfg_data.DATA_PATH,
    ...
    print(
        "dataset is {}, training images num is {}".format(
            datasetname, train_set.__len__()
        )
    )
...
def loading_data(datasetname, val_interval, distributed, is_main):
    ...
    train_loader, sampler_train = createTrainData(
        datasetname, Dataset, cfg_data, distributed
    )
    restore_transform = createRestore(cfg_data.MEAN_STD)

    Dataset = dataset.TestDataset
    val_loader = createValTestData(
        datasetname, Dataset, cfg_data, val_interval, True, is_main, mode="val"
    )
...
def createValTestData(
    datasetname, Dataset, cfg_data, frame_interval, skip_flag, is_main, mode="val"
):
    if is_main:
        img_transform = standard_transforms.Compose(
            [
        ...
        with open(
            os.path.join(
                cfg_data.DATA_PATH, eval("cfg_data.{}_LST".format(mode.upper()))
            ),
            "r",
        ) as txt:
            scene_names = txt.readlines()
        ...
        if datasetname == "MovingDroneCrowd":
            ...
            for scene_name in scene_names:
                ...
                if "/" in scene_name:
                    ...
                    clip_names = [
                        clip_name
                        for clip_name in os.listdir(root)
                        if not "." in clip_name
                    ]
                    clip_names.sort()
...
def loading_testset(datasetname, test_interval, skip_flag, mode="test"):
    ...
    test_loader = createValTestData(
        datasetname, Dataset, cfg_data, test_interval, skip_flag, True, mode=mode
    )
