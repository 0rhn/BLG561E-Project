    ...
    def __init__(
        ...
        for scene_name in scene_names:
            if datasetname == "MovingDroneCrowd":
                ...
                img_path, label = UAVVIC_ImgPath_and_Target(
                    base_path, scene_name.strip()
                )
            else:
...
def HT21_ImgPath_and_Target(base_path, i):
    ...
    for img_id in img_ids:
        ...
        sigma = torch.min(box[:, 2:4], 1)[0] / 2.0
...
def SENSE_ImgPath_and_Target(base_path, i):
    ...
    gts = defaultdict(list)
    with open(
        root.replace("video_ori", "label_list_all") + ".txt", "r"
    ) as f:  # label_list_all_rmInvalid
        lines = f.readlines()
        for lin in lines:
    ...
    for img_id in img_ids:
        ...
        if ids.size(0) > 0:
            ...
            )  # torch.sqrt(((box_and_point[:,2]-box_and_point[:,0])/2)**2 + ((box_and_point[:,3]-box_and_point[:,1])/2)**2)
        else:
            sigma = torch.tensor([])
        img_path.append(single_path)
...
class TestDataset(data.Dataset):
    ...
    def __init__(
        ...
        if self.target:
            if datasetname == "MovingDroneCrowd":
                self.imgs_path, self.label = MDC_ImgPath_and_Target(
                    self.base_path, scene_name
                )
            elif datasetname == "UAVVIC":
                self.imgs_path, self.label = UAVVIC_ImgPath_and_Target(
                    self.base_path, scene_name
                )
            else:
                raise NotImplementedError
