...
from misc.inflation import DiamondKernelInflation


class ProcessSub(object):
    def __init__(self, T=0.1, K=51):
        self.T = T
        self.inf = DiamondKernelInflation(kernel_size=K)
...
class RandomEmptyFlow(object):
    def __call__(self, flow):
        if random.random() < 0.04:
            flow = numpy.zeros((flow.shape[0], flow.shape[1], flow.shape[2])).astype(
                numpy.float32
            )
        return flow
...
class ScalebyRate(object):
    ...
    def __call__(self, img, den):
        ...
        den = (
            np.array(den.resize((dst_den_w, dst_den_h), Image.BILINEAR))
            / real_rate_w
            / real_rate_h
        )
...
class ScaleByRateWithMin(object):
    ...
    def __call__(self, img, gt):  # dot, flow):
        ...
        gt["points"] = (
            gt["points"] * rate
        )  # cv2.resize(gt[1], (new_w, new_h), interpolation=cv2.INTER_NEAREST)
        return img, gt
...
def check_image(img, target, crop_size, max_size):
    ...
    if w < c_w or h < c_h:
        ...
        target["points"] = target["points"] + torch.tensor(
            [delta_w // 2, delta_h // 2], dtype=torch.float32
        )
...
if __name__ == "__main__":
    ...
    main_transform = Compose([ScaleByRateWithMin([0.8, 1.2], 1024, 512)])
