...
def save_results_more(
    ...
    for idx, tensor in enumerate(
        ...
        pred_color_map = cv2.applyColorMap(
            (255 * tensor[1] / (tensor[2].max() + 1e-10)).astype(np.uint8).squeeze(),
            cv2.COLORMAP_JET,
        )
        gt_color_map = cv2.applyColorMap(
            (255 * tensor[2] / (tensor[2].max() + 1e-10)).astype(np.uint8).squeeze(),
            cv2.COLORMAP_JET,
        )
        binar_color_map = cv2.applyColorMap(
            (255 * tensor[3] / (tensor[4].max() + 1e-10)).astype(np.uint8).squeeze(),
            cv2.COLORMAP_JET,
        )
        gt_matched_color_map = cv2.applyColorMap(
            (255 * tensor[4] / (tensor[4].max() + 1e-10)).astype(np.uint8).squeeze(),
            cv2.COLORMAP_JET,
        )
        weights_color_map = cv2.applyColorMap(
            (255 * tensor[5] / (tensor[5].max() + 1e-10)).astype(np.uint8).squeeze(),
            cv2.COLORMAP_JET,
        )
        ...
        pil_gt_matched = Image.fromarray(
            cv2.cvtColor(gt_matched_color_map, cv2.COLOR_BGR2RGB)
        )
        pil_weights = Image.fromarray(
            cv2.cvtColor(weights_color_map, cv2.COLOR_BGR2RGB)
        )
        ...
        count = 0
        for img in imgs:
            x, y = int(count % w_num) * (UNIT_W + 10), int(count // w_num) * (
                UNIT_H + 10
            )  # 左上角坐标，从左到右递增
...
def vis_results_more(
    ...
    for idx, tensor in enumerate(
        zip(img.cpu().data, pred_map, gt_map, binar_map, threshold_matrix)
    ):
        if idx > 1:  # show only one group
            break
        ...
        binar_color_map = cv2.applyColorMap(
            (255 * tensor[3]).astype(np.uint8).squeeze(), cv2.COLORMAP_JET
        )
        threshold_color_map = cv2.applyColorMap(
            (255 * tensor[4] / (tensor[2].max() + 1e-10)).astype(np.uint8).squeeze(),
            cv2.COLORMAP_JET,
        )
        ...
        for i, box in enumerate(pred_boxes, 0):
            ...
            cv2.rectangle(
                pil_input, wh_LeftTop, wh_RightBottom, point_color, thickness, lineType
            )
        ...
        pil_threshold = Image.fromarray(
            cv2.cvtColor(threshold_color_map, cv2.COLOR_BGR2RGB)
        )
        ...
        x.extend(
            [
                ...
                pil_to_tensor(pil_binar.convert("RGB")),
...
def vis_results(
    exp_name, epoch, writer, restore, img, pred_map, gt_map, binar_map, boxes
):  # , flow):
    ...
    for idx, tensor in enumerate(zip(img.cpu().data, pred_map, gt_map, binar_map)):
        ...
        binar_color_map = cv2.applyColorMap(
            (255 * tensor[3] / (tensor[2].max() + 1e-10)).astype(np.uint8).squeeze(),
            cv2.COLORMAP_JET,
        )
...
def print_NWPU_summary(trainer, scores):
    ...
    print("    " + "-" * 20)
    print(
        "  [F1 %.4f Pre %.4f Rec %.4f] [mae %.2f mse %.2f], [val loss %.4f]"
        % (f1m_l, ap_l, ar_l, mae, mse, loss)
    )
    print("    " + "-" * 20)
    print(
        "[best] [model: %s] , [F1 %.4f Pre %.4f Rec %.4f] [mae %.2f], [mse %.2f], [nae %.4f]"
        % (
...
def update_model(trainer, scores):
    ...
    for key, data in scores.items():
        ...
        if data < train_record[key]:
            ...
            torch.save(
                to_saved_weight,
                os.path.join(
                    trainer.exp_path, trainer.exp_name, snapshot_name + ".pth"
                ),
            )
    ...
    latest_state = {
        ...
        "epoch": trainer.epoch,
        "i_tb": trainer.i_tb,
        "exp_path": trainer.exp_path,
        "exp_name": trainer.exp_name,
    }
    torch.save(
        latest_state,
        os.path.join(trainer.exp_path, trainer.exp_name, "latest_state.pth"),
    )
        ...
        if id1 is not None:
            for i, (id, centroid) in enumerate(zip(id1, kpts1)):
                cv2.putText(
                    out,
                    str(id),
                    (centroid[0] + margin + W0, centroid[1]),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5,
                    (0, 0, 255),
                    2,
...
def change2map(intput_map):
    ...
    vis_map = (intput_map - intput_map.min()) / (
        intput_map.max() - intput_map.min() + 1e-5
    )
...
def save_test_visual(
    visual_maps, imgs, scene_name, restor_transform, save_path, iter, rank
):
    visual_data = [visual_maps[:, i, :, :] for i in range(visual_maps.shape[1])]
    visual_data = [torch.stack(imgs, dim=0)] + visual_data
    save_visual_results(
        visual_data, restor_transform, os.path.join(save_path, scene_name), iter, rank
    )
...
def compute_metrics_all_scenes(scenes_pred_dict, scene_gt_dict, intervals):
    ...
    metrics = {
        "MAE": torch.zeros(scene_cnt, 2),
        "WRAE": torch.zeros(scene_cnt, 2),
        "MIAE": torch.zeros(0),
        "MOAE": torch.zeros(0),
    }
    for i, (pre_dict, gt_dict) in enumerate(zip(scenes_pred_dict, scene_gt_dict), 0):
        ...
        metrics["MIAE"] = torch.cat(
            [metrics["MIAE"], torch.abs(inflow_cnt[:, 0] - inflow_cnt[:, 1])]
        )
        metrics["MOAE"] = torch.cat(
            [metrics["MOAE"], torch.abs(outflow_cnt[:, 0] - outflow_cnt[:, 1])]
        )
    ...
    WRAE = (
        torch.sum(
            metrics["WRAE"][:, 0]
            * (metrics["WRAE"][:, 1] / (metrics["WRAE"][:, 1].sum() + 1e-10))
        )
        * 100
    )
...
def local_maximum_points(sub_pre, gaussian_maximun, radius=8.0):
